%step 2 is to find another Do
clc
clear all
close all
flag=1;
%for W=0.01:0.01:0.3
%for W=0.004:0.0001:0.006
for W=0.0052
%for W=0.04
W
save_W(1,flag)=W;
...
I=imread('hw3_5_f_noisy2_after_filter_1time.png');%periodic noise
...
%Do=sqrt((156-round(size(I,1)/2)+1)^2+(176-round(size(I,1)/2)+1)^2);
Do=sqrt((102-(round(size(I,1)/2)+1))^2+(82-(round(size(I,1)/2)+1))^2);
...
%Io = ButterworthBandReject (I,Do,W,n);
    ...
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    load('.\freq_image_noisy2_after_first_step.mat');
    I_fre_shift=freq_image_noisy2_after_first_step;
    ...
    imwrite(original_frequency,'hw3_5_f_noisy2_after_filter_2time_original_frequency.png');
...
%     max(max(original_frequency(1:end*2/5,1:end*2/5)))
    ...
    imwrite(output_frequency,'hw3_5_f_noisy2_after_filter_2time_output_frequency.png');
...
imwrite(I,'hw3_5_f_noisy2_after_filter_2time_original_image.png');
figure(4);imshow(Io_way2);title('output image');
%imwrite(Io_way2,'hw3_5_f_noisy2_after_filter_2time.png');
